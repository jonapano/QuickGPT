...
const Sidebar = ({ isMenuOpen, setIsMenuOpen }) => {
  ...
  return (
    <div className={`flex flex-col h-screen min-w-72 p-5
      dark:bg-gradient-to-b from-[#242124]/30 to-[#000000]/30 border-r border-[#80609F]/30 backdrop-blur-3xl transition-all duration-500 max-md:absolute left-0 z-1 ${!isMenuOpen && 'max-md:-translate-x-full'}`}>
      ...
      <div className='mt-4 flex flex-col flex-1 overflow-hidden'>
        {chats.length > 0 && (
          <p className='text-sm mb-2 sticky top-0 bg-inherit z-10'>Recent Chats</p>
        )}

        <div className='flex-1 overflow-y-auto pr-1 scrollbar-thin scrollbar-thumb-[#80609F]/30 scrollbar-track-transparent'>
          {chats
            .filter(chat =>
              chat.messages[0]
            ...
            .map(chat => (
              <div
                ...
                key={chat._id}
                className='p-2 px-4 my-3 dark:bg-[#57317C]/10 border border-gray-300 dark:border-[#80609F]/15 rounded-md cursor-pointer flex justify-between group'
              >
                <div>
      ...
      <div className='flex flex-col mt-4'>
        ...
        <div className='flex items-center gap-3 p-3 mt-4 border border-gray-300 dark:border-white/15 rounded-md cursor-pointer group'>
          ...
          {user && <img onClick={logout} className='h-5 cursor-pointer hidden not-dark:invert group-hover:block' src={assets.logout_icon} alt="logout" />}
        </div>
      </div>
